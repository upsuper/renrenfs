...
- (NSData *)icnsDataWithWidth:(int)width
{
    ...
    // format, but when NSAlphaNonpremultipliedBitmapFormat is used on 10.5 we 
    // get an error about invalid parameters for graphics context. We should 
    // try this again in future 10.5 releases in case it gets fixed. Until then,
    // maybe we should use vImageUnpremultiplyData_ARGB8888(...) on 10.5?
    if (version == 4 || version == 6) {
        format |= NSAlphaNonpremultipliedBitmapFormat;
    }
    NSBitmapImageRep* rep = [[NSBitmapImageRep alloc] 
                             // Single plane; just our bitmapData
    ...
    [self drawInRect:rect fromRect:NSZeroRect operation:NSCompositeCopy 
            fraction:1.0];
    ...
    if (version == 4) {
        ...
        vImagePermuteChannels_ARGB8888(&src, &dst, permuteMap, 
                                       kvImageDoNotTile);
    }
    ...
    NSData* data = [NSData dataWithBytes:*familyHandle 
                                  length:GetHandleSize(familyHandle)];
